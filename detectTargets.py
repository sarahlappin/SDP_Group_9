...
def getTargets():
    ...
    def addShadow():
        ...
        cv2.imshow("Target Selection", currentState[arrayCounter])

    def undoButton():
        nonlocal arrayCounter
        if arrayCounter > 0:
            arrayCounter -= 1
        nonlocal dst
        addShadow()
    ...
    def quitButton():
        ...
        text = tkinter.Label(windowSure,
                             text="Are you sure you don't want to add any other areas to the selection?",
    ...
    while not closeWindow:
        roi = cv2.selectROI('Target Selection', currentState[arrayCounter], False, False)
        x, y, w, h = roi
    ...
    def clickCoord(event, mouseX, mouseY, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            nonlocal numberedTargets
            nonlocal targetCoordinates
            nonlocal imageStates
            nonlocal arrayCounter
            nonlocal arrayStates
            nonlocal dst
    ...
    cv2.setMouseCallback("Target Selection", clickCoord)
